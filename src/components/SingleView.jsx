...
export default function SingleView() {
  ...
  // If product is not yet loaded, show a loading spinner
  if (!product) return (<div className="loading-spinner"></div>);

  const { user } = product;
  const title = product.description ?? product.alt_description;
  const style = {
    backgroundImage: `url(${product.urls?.regular || ''})` // Set product image as background
  };
    ...
    <article className="bg-white center mw7 ba b--black-10 mv4">
      <div className="pv2 ph3">
        <div className="flex items-center">
          <img src={user?.profile_image?.medium} className="br-100 h3 w3 dib" alt={user?.instagram_username || 'User profile'} />
      ...
      <div className="pa3 flex justify-end">
        ...
        <AddToCart product={product} /> {/* Add the product to the cart */}
      </div>
    </article>
  );
}
